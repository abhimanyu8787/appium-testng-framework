...
public class CartPage extends AndroidGenericMethods{
    ...
    public double calculateTotalPriceOfProducts() {
        ...
        for(WebElement element : productPriceList) {
            sum += Double.valueOf(element.getText().substring(1));
        }
        return sum;
    }
    
    public double getTotalPurchaseAmount() throws NumberFormatException, Exception {
        return Double.valueOf(getElementText(totalPrice).substring(1));
    }
